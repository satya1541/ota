...
export function LiveSerialMonitor({ devices, className }: LiveSerialMonitorProps) {
  ...
  const connect = useCallback(() => {
    if (!selectedDevice) {
      toast.error(t('serial_monitor.select_device_first'));
      return;
    }
    ...
    try {
      ...
      ws.onopen = () => {
        setIsConnected(true);
        setIsStreaming(true);
        toast.success(t('serial_monitor.connected_to_stream'));
      ...
      ws.onerror = () => {
        toast.error(t('serial_monitor.connection_error'));
      };
      ...
      toast.error(t('serial_monitor.failed_to_connect'));
    }
  }, [selectedDevice, t]);
  ...
  const exportLogs = () => {
    ...
    toast.success(t('serial_monitor.logs_exported'));
  };
  ...
  const copyLogs = async () => {
    ...
    setCopied(true);
    toast.success(t('serial_monitor.logs_copied'));
  ...
  return (
    <Card className={`border-none shadow-lg ring-1 ring-border bg-card ${className}`}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-bold flex items-center gap-2">
            ...
            {t('serial_monitor.header')}
            {isConnected && (
              <Badge className="bg-emerald-500/20 text-emerald-500 dark:text-emerald-400 border-emerald-500/30 ml-2">
                <Wifi className="h-3 w-3 mr-1" />
                {t('serial_monitor.connected')}
              </Badge>
            )}
          </CardTitle>
          <div className="flex items-center gap-2">
            <Select value={selectedDevice} onValueChange={setSelectedDevice}>
              <SelectTrigger className="w-[180px] h-8 bg-muted/50 border-none ring-1 ring-border text-xs">
                <SelectValue placeholder={t('serial_monitor.select_device')} />
              </SelectTrigger>
              <SelectContent>
                {devices.map(device => (
            ...
            {isStreaming ? (
              <Button
                ...
                size="sm"
                onClick={disconnect}
                className="h-8 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-500/10"
              >
                <Square className="h-4 w-4 mr-1" />
                {t('serial_monitor.stop')}
              </Button>
            ) : (
              <Button
                variant="ghost"
                ...
                className="h-8 text-emerald-500 hover:text-emerald-600 dark:text-emerald-400 dark:hover:text-emerald-300 hover:bg-emerald-500/10"
              >
                <Play className="h-4 w-4 mr-1" />
                {t('serial_monitor.start')}
              </Button>
            )}
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={t('serial_monitor.search_logs')}
              className="pl-9 h-9 bg-muted/50 border-none ring-1 ring-border text-sm"
            />
          </div>

          <Select value={filter} onValueChange={setFilter}>
            <SelectTrigger className="w-[120px] h-9 bg-muted/50 border-none ring-1 ring-border text-xs">
              <Filter className="h-3 w-3 mr-2" />
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t('serial_monitor.all_levels')}</SelectItem>
              <SelectItem value="info">{t('serial_monitor.info')}</SelectItem>
              <SelectItem value="warn">{t('serial_monitor.warning')}</SelectItem>
              <SelectItem value="error">{t('serial_monitor.error')}</SelectItem>
              <SelectItem value="debug">{t('serial_monitor.debug')}</SelectItem>
            </SelectContent>
          </Select>
        ...
        <div
          ref={scrollRef}
          className="h-[350px] bg-zinc-950/90 dark:bg-black/40 rounded-xl ring-1 ring-border overflow-auto font-mono text-xs"
        >
          {filteredLogs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground/50">
              <Terminal className="h-12 w-12 mb-3 opacity-50" />
              <p>{t('serial_monitor.no_logs_yet')}</p>
              <p className="text-[10px] mt-1">
                {isStreaming ? t('serial_monitor.waiting_for_output') : t('serial_monitor.start_streaming_prompt')}
              </p>
            </div>
          ) : (
            <div className="p-3 space-y-1">
              <AnimatePresence initial={false}>
                {filteredLogs.map(log => (
                  ...
                  >
                    <span className="text-zinc-500 dark:text-white/30 flex-shrink-0">
                    ...
                    <span className="text-zinc-300 dark:text-white/80 break-all">{log.message}</span>
                  </motion.div>
                ))}
              </AnimatePresence>
            </div>
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            <span>{filteredLogs.length} / {logs.length} {t('serial_monitor.logs')}</span>
            {selectedDevice && (
          ...
          <button
            ...
            className={`flex items-center gap-1 ${autoScroll ? "text-accent" : "text-muted-foreground"}`}
          >
            {autoScroll ? t('serial_monitor.auto_scroll_on') : t('serial_monitor.auto_scroll_off')}
          </button>
        </div>
      </CardContent>
    </Card>
