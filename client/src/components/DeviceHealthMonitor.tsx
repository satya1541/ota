...
export function DeviceHealthMonitor({ devices, className }: DeviceHealthMonitorProps) {
  ...
  return (
    <Card className={`border-none shadow-lg ring-1 ring-white/10 bg-black ${className}`}>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4].map(bar => (
                  <div
                    key={bar}
                    className={`w-1.5 rounded-sm ${bar <= getSignalBars(healthStats.avgSignal)
                      ? "bg-blue-400"
                      : "bg-white/20"
                      }`}
                    ...
                    {device.signalStrength && (
                      <div className="flex items-center gap-0.5">
                        {[1, 2, 3, 4].map(bar => (
                          <div
                            key={bar}
                            className={`w-1 rounded-sm ${bar <= getSignalBars(device.signalStrength)
                              ? "bg-white/60"
                              : "bg-white/20"
                              }`}
