...
import { logsApi, DeviceLog } from "@/lib/api";
...
export function DeploymentAnalytics({ className }: DeploymentAnalyticsProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const pieData = [
      { name: t('overview.deploy'), value: actionCounts.deploy, color: "#8b5cf6" },
      { name: t('overview.report'), value: actionCounts.report, color: "#22c55e" },
      { name: t('overview.check'), value: actionCounts.check, color: "#3b82f6" },
      { name: t('overview.download'), value: actionCounts.download, color: "#f59e0b" },
      { name: t('overview.rollback'), value: actionCounts.rollback, color: "#ef4444" },
    ].filter(d => d.value > 0);
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-card border border-border/50 shadow-2xl backdrop-blur-md px-3 py-2 text-xs">
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="border-none shadow-lg ring-border/50 bg-card/50 shadow-2xl backdrop-blur-md">
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">{analytics.totalDeploys}</p>
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{t('overview.total_deploys')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] uppercase tracking-widest text-emerald-400/60">{t('overview.successful')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] uppercase tracking-widest text-red-400/60">{t('overview.failed')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] uppercase tracking-widest text-blue-400/60">{t('overview.success_rate')}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-lg ring-border/50 bg-card/50 shadow-2xl backdrop-blur-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold flex items-center gap-2">
              ...
              {t('overview.daily_deployment_trend')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex justify-center gap-6 mt-2 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-foreground/60">{t('overview.success')}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded bg-red-500" />
                <span className="text-foreground/60">{t('overview.failed')}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-lg ring-border/50 bg-card/50 shadow-2xl backdrop-blur-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold flex items-center gap-2">
              ...
              {t('overview.action_distribution')}
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-none shadow-lg ring-border/50 bg-card/50 shadow-2xl backdrop-blur-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-bold flex items-center gap-2">
              ...
              {t('overview.top_deployed_versions')}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {analytics.versionData.length === 0 ? (
                <p className="text-center text-muted-foreground py-8">{t('overview.no_deployment_data')}</p>
              ) : (
                  ...
                  <div key={item.version} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-muted-foreground">{item.count} {t('overview.deploys')}</span>
                      </div>
