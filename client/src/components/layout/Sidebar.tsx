...
export function Sidebar({ onClear }: SidebarProps) {
  ...
  return (
    <div className="flex h-screen w-64 flex-col border-r border-sidebar-border bg-transparent backdrop-blur-xl text-sidebar-foreground">
      <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-6">
        <Link href="/dashboard" className="flex items-center gap-3 font-semibold group">
          ...
          <div className="flex flex-col">
            <span className="text-lg tracking-tight font-black leading-none text-white">Universal OTA</span>
          ...
          {navItems.map((item) => (
            <Link key={item.href} href={item.href}>
              <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                className={cn(
                  "group flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-bold transition-all relative overflow-hidden backdrop-blur-md",
                  location === item.href 
              ...
              >
                <item.icon className={cn(
                  ...
                  location === item.href ? "text-accent scale-110" : "text-white/30 group-hover:text-accent group-hover:scale-110"
                )} />
