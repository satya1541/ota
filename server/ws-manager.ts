...
export class WebSocketManager {
  ...
  public broadcastToDevice(mac: string, message: any) {
  ...
  // Broadcast console message to subscribed clients
  public broadcastToConsole(mac: string, message: any) {
    const normalizedMac = mac.replace(/[:-]/g, "").toUpperCase();
    const msgString = JSON.stringify(message);
  ...
  public broadcastConsoleOutput(mac: string, output: { type: "stdout" | "stderr" | "info"; message: string; timestamp?: Date }) {
    const normalizedMac = mac.replace(/[:-]/g, "").toUpperCase();
    const message = JSON.stringify({
      type: "console-output",
      mac: normalizedMac,
