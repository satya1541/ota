...
import type { IStorage, InsertStagedRollout, InsertDeviceHeartbeat } from "./storage";
...
export class DbStorage implements IStorage {
  ...
  async getUserByPin(pin: string): Promise<User | undefined> {
    const result = await this.db.select().from(users).where(eq(users.pin, pin));
    // Since PIN is not unique in DB schema (though logic enforces it), return first match
    return result[0];
  }
