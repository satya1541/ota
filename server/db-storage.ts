...
export class DbStorage implements IStorage {
  ...
  async getDeviceByMac(macAddress: string): Promise<Device | undefined> {
    const result = await this.db.select().from(devices).where(eq(devices.macAddress, macAddress));
    return result[0];
  }

  async createDevice(insertDevice: InsertDevice): Promise<Device> {
    await this.db.insert(devices).values(insertDevice);
    const result = await this.getDeviceByMac(insertDevice.macAddress);
  ...
  async updateDevice(id: string, updates: Partial<InsertDevice>): Promise<Device | undefined> {
    await this.db.update(devices).set(updates).where(eq(devices.id, id));
    return this.getDevice(id);
  }

  async updateDeviceByMac(macAddress: string, updates: Partial<InsertDevice>): Promise<Device | undefined> {
    await this.db.update(devices).set(updates).where(eq(devices.macAddress, macAddress));
    return this.getDeviceByMac(macAddress);
  }
  ...
  async getDeviceLogs(deviceId?: string): Promise<DeviceLog[]> {
    if (deviceId) {
      return this.db.select().from(deviceLogs)
        .where(eq(deviceLogs.deviceId, deviceId))
        .orderBy(desc(deviceLogs.createdAt));
    }
    return this.db.select().from(deviceLogs).orderBy(desc(deviceLogs.createdAt));
  }

  async createDeviceLog(log: InsertDeviceLog): Promise<DeviceLog> {
    await this.db.insert(deviceLogs).values(log);
