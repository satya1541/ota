...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/auth/login", validateBody(loginSchema), async (req, res) => {
    try {
      const { pin } = req.validatedBody as { pin: string };
      // Hash PIN before querying because DB stores hashed PINs
      const hashedPin = hashPin(pin);

      // For PIN-only login, we find the user by hashed PIN
      const user = await storage.getUserByPin(hashedPin);
  ...
  app.post("/api/auth/register", validateBody(registerSchema), async (req, res) => {
    try {
      ...
      const hashedPin = hashPin(pin);
      const existingUser = await storage.getUserByPin(hashedPin);

      if (existingUser) {
        return res.status(400).json({ error: "PIN already exists" });
      }

      // Auto-generate a username since it's required by the User type but optional in DB
      const username = `User-${Math.floor(Math.random() * 10000)}`;

      const user = await storage.createUser({ username, pin: hashedPin });
      logger.info('New user registered with PIN', { userId: user.id });
      res.json({ user: { id: user.id, username: user.username } });
    } catch (error) {
  ...
  app.get("/api/devices/:mac/commands", async (req, res) => {
    try {
      const commands = await storage.getDeviceCommands(req.params.mac);
      res.json(commands);
    } catch (error) {
  ...
  // POST create a command for a device
  app.post("/api/devices/:mac/commands", async (req, res) => {
    try {
      const { command, payload, expiresAt } = req.body;
      const device = await storage.getDeviceByMac(req.params.mac);
      if (!device) {
        return res.status(404).json({ error: "Device not found" });
      }

      const newCommand = await storage.createDeviceCommand({
        macAddress: req.params.mac,
        command,
        payload: payload || null,
        status: "pending",
        expiresAt: expiresAt ? new Date(expiresAt) : new Date(Date.now() + 5 * 60 * 1000),
      });

      // Broadcast command via WebSocket if device is online
      getWebSocketManager().broadcastToDevice(req.params.mac, {
        type: "command",
        command: newCommand
      });

      // Audit log
      await createAuditLog("send_command", "device", device.id, device.name, { command, payload }, req, "info");
  ...
  // POST report command result (ESP32 endpoint)
  app.post("/ota/command/report", async (req, res) => {
    try {
      const { id, status, response } = req.body;
      const updated = await storage.updateDeviceCommand(id, {
        status,
      ...
      if (updated) {
        ...
        getWebSocketManager().broadcastToConsole(updated.macAddress, {
          type: "command-ack",
      ...
      res.status(500).json({ error: "Failed to report command status" });
    }
  });

  // GET pending commands (ESP32 polling endpoint)
  app.get("/ota/commands", async (req, res) => {
    const mac = req.query.mac as string;
    if (!mac) return res.status(400).json({ error: "MAC required" });

    try {
      const commands = await storage.getPendingCommands(mac);
      res.json(commands);
    } catch (error) {
  ...
  app.post("/api/deploy", validateBody(deploySchema), async (req, res) => {
    try {
      ...
      const results: { deviceId: string; mac: string; status: string; message: string; errorType?: string }[] = [];
        ...
        try {
          ...
          if (!device) {
            results.push({
              ...
              mac: macAddress,
              status: "failed",
              message: "Device not found. Please verify the device is registered.",
          ...
          if (updateQueue.isDeviceUpdating(macAddress)) {
            results.push({
              ...
              mac: macAddress,
              status: "failed",
              message: "Device is currently processing another update. Use 'Reset Activity' button to clear stuck states.",
          ...
          if (errorMessage.includes('already being updated')) {
            errorType = "DEVICE_BUSY";
            userMessage = "Device is busy with another update. Try 'Reset Activity' to clear the state.";
          } else if (errorMessage.includes('Same version was recently deployed')) {
            errorType = "DUPLICATE_DEPLOYMENT";
            userMessage = `Version ${version} was recently deployed. Wait 5 minutes or reset device activity.`;
          } else if (errorMessage.includes('not found')) {
            errorType = "DEVICE_NOT_FOUND";
            userMessage = "Device not found in the system.";
          }
  ...
  app.post("/api/rollback/:macAddress", async (req, res) => {
    try {
      ...
      // Check if the previous firmware version exists in the firmware table
      const previousFirmware = await storage.getFirmware(device.previousVersion);
      if (!previousFirmware) {
        return res.status(400).json({
          error: "Previous firmware version not available for rollback",
          message: `Firmware version ${device.previousVersion} does not exist. Please upload this version before attempting rollback.`
        });
      }
  ...
  // POST reset device activity status
  app.post("/api/reset-activity/:macAddress", async (req, res) => {
    try {
      const validationResult = macAddressSchema.safeParse(req.params.macAddress);
      if (!validationResult.success) {
        return res.status(400).json({ error: "Invalid MAC address format" });
      ...
      // Reset device activity to idle state
      await storage.updateDeviceByMac(macAddress, {
        targetVersion: "",
        otaStatus: "idle",
        lastOtaCheck: new Date(),
      ...
      await storage.createDeviceLog({
        ...
        status: "success",
        message: "Device activity reset to idle",
      });

      // Audit log for activity reset
      await createAuditLog("reset_activity", "device", device.id, device.name,
        { previousStatus: device.otaStatus, newStatus: "idle" }, req, "info");

      logger.info('Device activity reset', { macAddress, previousStatus: device.otaStatus });
      res.json({ success: true, message: "Device activity reset successfully" });
    } catch (error) {
      logger.error('Activity reset failed', { error: error instanceof Error ? error.message : 'Unknown error', macAddress: req.params.macAddress });
      res.status(500).json({ error: "Failed to reset device activity" });
    }
  });
